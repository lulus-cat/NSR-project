/**
 * 근무지 지오펜스 — 병동에 들어오면 녹음을 켜고, 나가면 끈다.
 *
 * 왜 필요한가
 * ----------
 * 듀티표 기반 자동 녹음은 **근무표 시각**만 안다. 실제 간호사의 하루는
 ...
 * 동작
 * ----
 *   근무지 반경 안으로 들어옴 → 녹음 시작 (근무일일 때만)
 *   반경 밖으로 나감        → 녹음 정지
 *
 * 근무일 판정을 하는 이유: 오프 날 병원 근처를 지나가거나 진료 보러 갔을 때
 * 녹음이 켜지면 안 된다. 오늘 또는 어제(나이트가 자정을 넘는다) 근무가
 * 있을 때만 켠다. 출근 전 오버타임은 이 판정 안에서 자연히 덮인다 —
 * 근무일에 일찍 도착하면 그 순간부터 녹음이니까.
 *
 * 정직한 한계
 * ----------
 * 안드로이드 14부터 백그라운드에서 마이크 포그라운드 서비스 시작이 제한된다.
 * 지오펜스 진입이 앱을 깨워도 녹음 시작이 막힐 수 있다. 그 경우 앱을 한 번
