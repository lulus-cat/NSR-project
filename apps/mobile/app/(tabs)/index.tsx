...
export default function Home() {
  ...
  useFocusEffect(
    useCallback(() => {
      void load();
      // 서버와 한 번 맞춘다 — 안 보낸 근무를 올리고, AI 가 써 둔 것을 받아 온다.
      // 누를 버튼을 두지 않는다. 예전에는 설정의 '결과 받기' 를 안 누르면 보고서도
      // 카드도 폰에 영영 안 들어왔고, 그 사실이 화면 어디에도 없었다.
      void syncWithServer()
        .then((out) => {
          if (out?.got && out.got.reports + out.got.cards > 0) void load();
        })
        .catch(() => {});
    }, [load]),
  );
