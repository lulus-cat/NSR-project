...
type Mode = "sets" | "reports" | "transcripts";
              ...
              sets.map(([shiftId, setCards]) => {
                ...
                return (
                  <View key={shiftId || "none"}>
                    <Pressable
                      accessibilityRole="button"
                      accessibilityLabel={`${setTitle(shiftId || undefined)} 외우기`}
                      onPress={() => router.push(`/cards/${encodeURIComponent(shiftId || "none")}`)}
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        gap: space.md,
                    ...
                    >
                      ...
                      {/* 줄을 누르면 외우기로 간다. 목록을 펼쳐 보는 것은 화살표로 따로 */}
                      <Pressable
                        accessibilityRole="button"
                        accessibilityLabel={open ? "목록 접기" : "목록 펼치기"}
                        hitSlop={12}
                        onPress={(e) => {
                          e.stopPropagation();
                          setOpenSet(open ? null : shiftId);
                        }}
                      >
                        <Ionicons
                      ...
                      ? setCards.map((c) => (
                          ...
                          >
                            ...
                            {c.sourceIds
                              ...
                              .map((src) =>
                                src ? (
                                  <Text key={src.id} style={[type.small, { color: t.textMuted }]}>
                                    더 볼 자료 · {src.name} ({src.publisher})
                                  </Text>
                                ) : null,
                              )}
                          </View>
