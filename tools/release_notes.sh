...
폰에서 아래 `.apk` 를 눌러 받은 뒤 열면 설치돼요.
**기존 앱 위에 덮어 설치**하면 녹음·전사본·단어장이 그대로 남아요. 지우지 마세요.

처음 설치할 때 "출처를 알 수 없는 앱" 허용을 한 번 물어봐요.

## 처음 켜면 할 일

1. 첫 안내를 읽고 넘어가요 (법적 사항이 들어 있어요)
2. **설정 → 전사**에서 어디서 글자로 바꿀지 골라요. 기본은 티로예요.
   열쇠 하나만 넣으면 돼요. 이걸 안 하면 글자로 못 바꿔요.
3. **설정 → 필수 기능**에서 AI 조합을 고르고 열쇠를 넣어요.
4. 듀티표를 넣으면 근무 시간에 저절로 기록해요.
