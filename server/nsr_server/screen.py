...
#
# 그래서 경계를 손으로 적는다. 자바스크립트의 `\b` 와 같은 뜻이다:
# "앞뒤가 영문·숫자·밑줄이 아니면 경계" — 한글은 경계로 친다.
EDGE = r"(?<![0-9A-Za-z_])"
EDGE_END = r"(?![0-9A-Za-z_])"

PATTERNS: dict[str, re.Pattern[str]] = {
    # 주민등록번호 6-7. 뒷자리를 별로 가린 것(940101-2******)도 잡는다.
    "rrn": re.compile(EDGE + r"\d{6}\s*[-–]\s*[1-4][\d*]{6}(?![\d*])"),
    # 휴대전화(010-1234-5678·010.1234.5678·01012345678)와 일반전화(02-123-4567)
    "phone": re.compile(
        EDGE + r"(?:01[016-9]|0(?:2|[3-6][1-5]))[-–.\s]?\d{3,4}[-–.\s]?\d{4}" + EDGE_END
    ),
    # 등록번호 — 낱말이 앞에 붙으면 5자리부터, 아니면 8자리 이상 연속 숫자
    "mrn_labeled": re.compile(
        r"(?:등록번호|차트번호|환자번호|아이디|ID)\s*[:는은]?\s*\d{5,}", re.I
    ),
    "mrn": re.compile(EDGE + r"\d{8,}" + EDGE_END),
    # 이메일. 앞은 한글이어도 되고(메일은hong@…), 아이디에 한글은 안 넣는다.
    "email": re.compile(EDGE + r"[A-Za-z0-9._+-]+@[\w-]+\.[\w.-]+" + EDGE_END),
}
