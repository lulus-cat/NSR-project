...
| 로그인 | **구글로 로그인** 버튼 (8번을 켜 뒀을 때) |
| 비상문 | '열쇠로 잇기' 를 펴서 3번의 `NSR_DEVICE_TOKEN` 붙여넣기 |

구글 로그인을 켜 두면 사람이 열쇠를 옮겨 적을 일이 없다. 폰마다 열쇠가 따로
발급되므로 폰을 하나 더 써도 그냥 로그인하면 된다.

## 8. 구글 로그인 켜기 (권장)

열쇠를 붙여넣는 일이 사라진다. 폰과 커넥터 양쪽에 적용된다.
