...
- **챗지피티** — 설정 → 커넥터 → 새 커넥터 → 위 주소.
  개발자 모드(설정 → 보안 → 개발자 모드)를 켜면 **쓰는 도구까지** 다 쓴다.
  안 켜도 붙기는 하는데, 그때는 `search`·`fetch` 둘로 **읽기만** 된다 —
  보고서를 써 넣거나 사전에 말을 넣는 일은 못 한다.

| 무엇 | 클로드 | 챗지피티 (개발자 모드) | 챗지피티 (그냥) |
| --- | --- | --- | --- |
| 근무·문장 읽기 | O | O | O (`search`·`fetch`) |
| 보고서 쓰기 | O | O | X |
| 사전·교정·근무 체온 넘기기 | O | O | X |

챗지피티는 개발자 모드가 아니면 `search` 와 `fetch` 가 없는 서버를 **아예 안
받는다.** 그래서 이 서버는 그 둘을 규격대로 낸다(`store.search_docs` ·
`store.fetch_doc`). 새로 여는 자료는 없다 — 이미 도구로 열어 둔 것에 이름표를
붙여 그 규격으로 내주는 것뿐이다.
