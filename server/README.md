...
앱 설정 → 분석 서버에 **서버 주소**(`https://nsr.example.com`)를 넣고 저장한다.
그 다음 열쇠는 사람이 옮겨 적지 않는다. 서버에서 QR 을 만들어 찍는다.

```bash
sudo -u nsr /home/nsr/venv/bin/python -m nsr_server.pair
```

(환경변수를 읽어야 하므로 서비스와 같은 방식으로 돌린다 — 아래 '안 될 때' 참고)

나오는 것은 주소 두 개와 터미널 QR 이다.

1. `https://nsr.example.com/pair/<쪽지>/qr` 을 **컴퓨터 브라우저**로 연다 → 큰 QR
2. **폰 카메라로 찍고** 뜨는 알림을 누른다 → '앱 열기' → 앱이 "연결됐어요"
3. 끝. 쪽지는 15분 뒤 사라지고 한 번 쓰면 없어진다.

QR 이 안 찍히면 폰 브라우저에 `https://nsr.example.com/pair/<쪽지>` 를 직접 연다.

**열쇠가 QR 에 들어 있지 않다.** QR 에 있는 것은 15분짜리 일회용 쪽지이고, 폰이
그것을 HTTPS 로 열쇠와 바꾼다. 열쇠는 폰이 주우러 온 순간에 만들어지므로, 쪽지를
안 쓰면 서버에도 아무것도 안 남는다.
