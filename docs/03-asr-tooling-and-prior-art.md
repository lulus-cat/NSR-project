...
그래서 앱은 **모델을 하나로 고정하지 않는다.** 목록과 판단 근거는
`packages/core/src/transcription/models.ts`에 있고, 앱의 "전사 모델" 화면에서
받고·고르고·지운다. 원본 `ggml-small`을 그대로 쓰면 CER 18%인데,
그 18%가 어디에 몰리느냐가 문제다 — 약물명과 수치에 몰리면 쓸 수 없는 시스템이 된다.

한국어 파인튜닝 모델의 **URL은 코드에 박아 두지 않았다.** 모델은 사라지고
이름이 바뀌고 라이선스가 달라진다. 죽은 링크를 남기는 것보다 넣는 방법을
화면에서 안내하는 편이 오래간다. HuggingFace의 한국어 파인튜닝 모델은
whisper.cpp의 `models/convert-h5-to-ggml.py`로 ggml로 바꿔
"직접 추가"에 등록하면 된다.

속도는 **적어 두지 않는다.** 폰마다 몇 배씩 차이가 나기 때문이다. 대신
small 대비 상대 속도만 두고, 실제 전사에서 이 기기의 값을 한 번 재서
(`recordSpeedSample`) 나머지를 환산한다. 남의 폰 벤치마크를 내 폰 숫자인 것처럼
보여주지 않는다.
